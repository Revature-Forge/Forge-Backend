...
public class PortfolioTest {
  ...
  SkillRepo skillRepo;
	...
	public void setup() {
		mvc = MockMvcBuilders
				.standaloneSetup(new PortfolioController(repo, aboutMeRepo, certificationRepo, educationRepo,
						equivalencyRepo, gitHubRepo, honorRepo, projectRepo, workExperienceRepo, workHistoryRepo, matrixRepo, skillRepo))
				.build();
	}
    ...
    Matrix matrix = new Matrix(1, "Languages", skills, port.get());
    matrices.add(matrix);
    given(matrixRepo.findByPortfolio(port.get())).willReturn(matrices);
